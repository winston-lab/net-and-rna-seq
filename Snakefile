...
rule all:
    input:
        ...
        #gene ontology
        expand(expand("gene_ontology/transcripts/{condition}-v-{control}/libsizenorm/{{category}}/{condition}-v-{control}_{{assay}}-libsizenorm-transcripts-{{category}}-{{direction}}-gene-ontology-results.tsv",
                      zip,
                      condition=conditiongroups,
                      control=controlgroups),
                assay=ASSAY,
                category=["genic", "antisense", "convergent", "divergent"],
                direction=["up", "down", "unchanged"]) if config["called_transcripts_gene_ontology"] and comparisons else [],
        expand(expand("gene_ontology/transcripts/{condition}-v-{control}/spikenorm/{{category}}/{condition}-v-{control}_{{assay}}-spikenorm-transcripts-{{category}}-{{direction}}-gene-ontology-results.tsv",
                      zip,
                      condition=conditiongroups_si,
                      control=controlgroups_si),
                assay=ASSAY,
                category=["genic", "antisense", "convergent", "divergent"],
                direction=["up", "down", "unchanged"]) if config["called_transcripts_gene_ontology"] and comparisons_si else [],
        expand(expand("gene_ontology/{{annotation}}/{condition}-v-{control}/libsizenorm/{condition}-v-{control}_{{assay}}-libsizenorm-{{annotation}}-{{direction}}-gene-ontology-results.tsv",
                      zip,
                      condition=conditiongroups,
                      control=controlgroups),
                assay=ASSAY,
                direction=["up", "down", "unchanged"],
                annotation=[k for k,v in config["differential_expression"]["annotations"].items() if v["run_gene_ontology"] and v["gene_ontology_mapping_file"]]) if config["differential_expression"]["annotations"] and comparisons else [],
        expand(expand("gene_ontology/{{annotation}}/{condition}-v-{control}/spikenorm/{condition}-v-{control}_{{assay}}-spikenorm-{{annotation}}-{{direction}}-gene-ontology-results.tsv",
                      zip,
                      condition=conditiongroups_si,
                      control=controlgroups_si),
                assay=ASSAY,
                direction=["up", "down", "unchanged"],
                annotation=[k for k,v in config["differential_expression"]["annotations"].items() if v["run_gene_ontology"] and v["gene_ontology_mapping_file"]]) if config["differential_expression"]["annotations"] and comparisons_si else [],
        expand(expand("gene_ontology/transcripts/{condition}-v-{control}/libsizenorm/{condition}-v-{control}_{{assay}}-libsizenorm-transcripts-gsea-results.tsv",
                      zip,
                      condition=conditiongroups,
                      control=controlgroups),
               assay=ASSAY) if config["called_transcripts_gene_ontology"] and comparisons else [],
        expand(expand("gene_ontology/transcripts/{condition}-v-{control}/spikenorm/{condition}-v-{control}_{{assay}}-spikenorm-transcripts-gsea-results.tsv",
                      zip,
                      condition=conditiongroups_si,
                      control=controlgroups_si),
               assay=ASSAY) if config["called_transcripts_gene_ontology"] and comparisons_si else [],
        expand(expand("gene_ontology/{{annotation}}/{condition}-v-{control}/libsizenorm/{condition}-v-{control}_{{assay}}-libsizenorm-{{annotation}}-gsea-results.tsv",
                      zip,
                      condition=conditiongroups,
                      control=controlgroups),
               assay=ASSAY,
