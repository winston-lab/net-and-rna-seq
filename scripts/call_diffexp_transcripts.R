...
main = function(intable, norm, sitable, samples, groups, condition, control, alpha, lfc,
    ...
    resdf.sig = resdf %>% filter(log10_padj> -log10(alpha))
    resdf.nonsig = resdf %>% filter(log10_padj<= -log10(alpha))
    maplot = ggplot() +
        geom_hline(yintercept = 0, color="black", linetype="dashed") +
        geom_point(data = resdf.nonsig, aes(x=mean_expr, y=log2_foldchange),
                   color="black", alpha=0.3, stroke=0, size=0.7) +
        geom_point(data = resdf.sig, aes(x=mean_expr, y=log2_foldchange),
    ...
    volcano = ggplot() +
        geom_point(data = resdf.nonsig, aes(x=log2_foldchange, y = log10_padj),
                   alpha=0.3, stroke=0, size=0.7) +
        geom_point(data = resdf.sig, aes(x=log2_foldchange, y = log10_padj),
                   alpha=0.3, stroke=0, size=0.7) +
    ...
    resdf %>%
        filter(log10_padj > -log10(alpha) & log2_foldchange > 0) %>%
